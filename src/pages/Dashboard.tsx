  ...
  return (
    <div className="pb-2 space-y-4">
      ...
      <div className="relative overflow-hidden" style={{ background: '#111111', minHeight: 220 }}>
        {carImgOk && (
          <img src="/ram/car.png" alt={vehicle.name} className="w-full object-contain"
            style={{ maxHeight: 240 }} onError={() => setCarImgOk(false)} />
        )}
        <div className="absolute inset-0 pointer-events-none" style={{
          background: 'linear-gradient(to bottom, rgba(17,17,17,0.3) 0%, rgba(17,17,17,0) 30%, rgba(13,13,13,0.5) 90%, #0d0d0d 100%)',
        }} />
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => onNavigate('commands')}
            className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-colors active:scale-95 ${
              status.doors_locked ? 'bg-green-950/50 border-green-800/60' : 'bg-yellow-950/50 border-yellow-700/60'
            }`}>
            {status.doors_locked
          ...
          <button onClick={() => onNavigate('commands')}
            className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-colors active:scale-95 ${
              status.engine_running ? 'bg-red-950/50 border-red-800/60 pulse-red' : 'bg-[#1a1a1a] border-[#2a2a2a]'
            }`}>
